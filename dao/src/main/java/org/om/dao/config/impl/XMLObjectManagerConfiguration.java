...
public class XMLObjectManagerConfiguration implements ObjectManagerConfiguration {
	...
	private void configure() throws JCRException {
		try {
			...
			persistenceContext = new JcrPersistenceContext(jcrSessionFactory.getSession());
			JcrPersistenceAdapterFactory jcrPersistenceDelegateFactory = new JcrPersistenceAdapterFactory();
