...
public class FieldMappingExtractorImpl implements FieldMappingExtractor {
   ...
   public MappedField extract(Field field) {
      ...
      final boolean validImplementationType = String.class.equals(implementationType) || EntityUtils.isEntity(implementationType) || ClassUtils.isPrimitiveOrAutoboxed(implementationType);
   ...
   private Mapping extractCollectionMapping(Mapped mapped, Collection collection, Field field) {
      ...
      if (!isSupportedType) {
         throw new MappingException("Collection field " + field.getName() + " is of type " + fieldType + " but only java.util.List, java.util.Map or java.util.Set are supported.");
      }
   ...
   /**
    * Extracts a {@link PropertyMapping} from the given field. TODO: this should go into a factory supporting different types of property mappings.
    * 
    * @param field
    * @return the extracted property mapping.
    * @throws MappingException if the mapping cannot be constructed or if there's no mapping.
    */
   ...
   /**
    * Returns the implementation type to be used for the given {@link Mapped} annotation and declared type. If the given annotation is null or the implementation type returned by it is null, this will
    * return the declared type, otherwise the specified implementation type.
    * 
    * @param mapped
    * @param declaredType
    * @return
